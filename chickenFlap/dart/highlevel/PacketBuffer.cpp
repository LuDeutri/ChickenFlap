...
size_t PacketBuffer::getBufferSize() const {
	return bufferSize;
}
...
bool PacketBuffer::assertNull(void* pointer) {
    ...
    error = true;
    DART_ERROR_RETURN(DART_ERROR_INTERNAL, false);
}
...
bool PacketBuffer::assertPosition(size_t length) {
	...
	if (position + length > size) {
		error = true;
		DART_ERROR_RETURN(DART_ERROR_BUFFER_OVERFLOW, false);
	}
...
void PacketBuffer::errorMemory() {
    error = true;
	DART_ERROR(DART_ERROR_INTERNAL);
}
...
void PacketBuffer::setSize(uint32_t size) {
	if (size > bufferSize) {
        error = true;
		DART_ERROR(DART_ERROR_BUFFER_OVERFLOW);
	}
...
void PacketBuffer::writeBoolean(bool value) {
	if (value)
		write((uint8_t)1);
	else
		write((uint8_t)0);
...
void PacketBuffer::write(uint8_t* buffer, size_t length) {
	write(buffer, length, 0, length);
}

void PacketBuffer::write(uint8_t* buffer, size_t length, uint32_t offset, size_t bufferSize) {
	if (!assertNull(buffer))
		return;
