...
MCP2515::MCP2515(const uint8_t spi, const uint8_t cs)
{
	SPI = spi;
    SPICS = cs;

	spi_begin(SPI);
	digitalMode(SPICS, ModeOutput);
    spi_end(SPI);
}
...
{
    startSPI();
    spi_transfer8(SPI, INSTRUCTION_RESET);
    endSPI();
...
{
    startSPI();
    spi_transfer8(SPI, INSTRUCTION_READ);
    spi_transfer8(SPI, reg);
    uint8_t ret = spi_transfer8(SPI, 0x00);
    endSPI();
...
void MCP2515::readRegisters(const REGISTER reg, uint8_t values[], const uint8_t n)
{
    ...
    for (uint8_t i=0; i<n; i++) {
        values[i] = spi_transfer8(SPI, 0x00);
    }
    endSPI();
}
...
{
    startSPI();
    spi_transfer8(SPI, INSTRUCTION_WRITE);
    spi_transfer8(SPI, reg);
...
{
    startSPI();
    spi_transfer8(SPI, INSTRUCTION_BITMOD);
    spi_transfer8(SPI, reg);
    spi_transfer8(SPI, mask);
    spi_transfer8(SPI, data);
    endSPI();
}
...
{
    startSPI();
    spi_transfer8(SPI, INSTRUCTION_READ_STATUS);
    uint8_t i = spi_transfer8(SPI, 0x00);
    endSPI();
